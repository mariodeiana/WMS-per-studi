...
@dataclass
class Practice:
    id:str; practice_type_code:str; client_id:str; period_start:str; period_end:str; due_date:str; requires_validation:bool=True; status:PracticeStatus=PracticeStatus.DA_FARE; tasks:list[Task]=field(default_factory=list); audit:list[AuditEvent]=field(default_factory=list); validated_by:Optional[str]=None; validated_at:Optional[datetime]=None; results:list[WorkResult]=field(default_factory=list); evidence:list[Evidence]=field(default_factory=list); validation_result_id:Optional[str]=None; closure_result_id:Optional[str]=None
