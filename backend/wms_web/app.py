...
class WMSRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path.startswith("/api/evidence/"):
            ...
            try:
                item=self.service.evidence_content(evidence_id);data=base64.b64decode(item.content_base64) if item.content_base64 else b""
                self.send_response(HTTPStatus.OK);self.send_header("Content-Type",item.content_type or "application/octet-stream");self.send_header("Content-Disposition",f'{"attachment" if disposition=="attachment" else "inline"}; filename="{item.filename}"');self.send_header("Content-Length",str(len(data)));self.end_headers();self.wfile.write(data)
            except KeyError as error:self._json({"error":str(error.args[0])},HTTPStatus.NOT_FOUND)
            return
        if path.startswith("/api/tasks/"):
            parts=[unquote(x) for x in path[len("/api/tasks/"):].split("/")];query=parse_qs(parsed.query);operator=query.get("operator",[""])[0];include_context=query.get("context",["0"])[0]=="1"
    ...
    def do_POST(self) -> None:
        ...
        practice_id,action=parts[2],parts[3];body=self._body();actor=str(body.get("actor") or "operatore web");outcome=str(body.get("outcome") or "");note=str(body.get("note") or "");attachments=body.get("attachments")
    ...
    def _body(self):
        try:length=int(self.headers.get("Content-Length","0"));return json.loads(self.rfile.read(length)) if length else {}
