...
- dati essenziali della pratica;
- avanzamento complessivo;
- Task completati;
- autore di ciascun Task;
- outcome, note ed evidenze prodotte;
- anomalie;
- area per outcome, nota ed evidenze della validazione;
- comando Valida pratica.

Non deve mostrare azioni manageriali di chiusura.
...
- riepilogo pratica;
- timeline e milestone rispetto alla scadenza finale X;
- avanzamento reale rispetto allo stato atteso;
- Task con assegnatario, stato, autore completamento, priorità e scadenza operativa;
- note, risultati ed evidenze;
- validatore e risultato della validazione;
- risultato della chiusura;
- fascicolo e audit in sezioni distinte;
- azioni manageriali coerenti: assegnazione, riapertura, chiusura quando consentita.
...
Deve mostrare in cima le pratiche che richiedono attenzione e permettere filtri/viste per stato.
...
- stato;
- avanzamento;
- milestone attesa alla data corrente;
- stato reale;
- anticipo/ritardo operativo;
- priority score/urgenza;
- responsabile/manager;
- blocchi o attese esterne.

Un osservatore deve capire immediatamente quali pratiche preoccupano il Manager e perché.
...
Gli offset appartengono al Template/Tipo Pratica e non sono valori globali.

Il WMS deve calcolare lo stato atteso alla data corrente e confrontarlo con lo stato reale. Questo scostamento è uno degli elementi fondamentali per stabilire la priorità.

## 12. Priority Engine

La priorità deve essere principalmente calcolata dinamicamente, non ridotta a un campo manuale Alto/Medio/Basso.

Input minimi:

- tempo residuo alla scadenza X;
- milestone attesa;
- avanzamento reale;
- scostamento reale/atteso;
- Task bloccati e dipendenze;
- criticità del tipo pratica;
- stato IN_ATTESA_ESTERNA;
- eventuale override del Manager.

La priorità deve ordinare sia il Portfolio Manager sia la Work Queue Operatore.

## 13. Task Context

Quando l'Operatore apre un Task, il WMS deve comporre il contesto operativo necessario senza costringerlo a cercare informazioni altrove.
...
- priorità;
- dipendenze;
- dati contestuali necessari;
- evidenze/documenti precedenti;
- documenti richiesti;
- area note/allegati/esito;
- completamento.
...
- risultati/evidenze precedenti;
- connector futuri verso gestionali.
...
L'evoluzione prevista è integrare la posta nel WMS anziché limitarsi ad aprire il client locale.

Il WMS dovrà poter preparare una mail contestualizzata con:

- destinatario/destinatari da marker/anagrafica;
- oggetto codificato e contestualizzato;
- corpo precompilato;
- allegati provenienti dal fascicolo;
- anteprima prima dell'invio;
- archiviazione del messaggio inviato nel fascicolo;
- audit dell'invio.
...
Le mail generate dal WMS devono poter essere correlate a Pratica e Task.

Meccanismi previsti:

- codice leggibile nell'oggetto, ad esempio `[WMS:P-2026-LIPE-001:TASK-LIPE-05]`;
- header applicativi dedicati quando possibile;
- Message-ID salvato;
- In-Reply-To e References.
...
Non tutti i Task richiederanno completamento manuale.
...
- firma completata;
- pagamento rilevato;
- raggiungimento di una data.

Stato previsto: IN_ATTESA_ESTERNA con reason code, ad esempio CLIENTE, ENTE, GESTIONALE, DOCUMENTO, FIRMA, PAGAMENTO.

Ogni avanzamento automatico deve essere auditabile e spiegabile.

## 18. Linguaggio e principi UI

Interfaccia professionale e sobria, adatta a uno studio professionale.

Priorità a leggibilità, densità informativa e chiarezza, non agli effetti grafici. Desktop-first con responsive ragionevole.

Stati, priorità e ritardi devono essere distinguibili senza dipendere esclusivamente dal colore.

Un'azione non consentita non deve apparire come apparentemente disponibile.
...
Per valutare realmente Dashboard e code operative servono dati realistici ma fittizi:
...
- 1 validatore;
- 1 manager;
- tipi pratica differenti, ad esempio LIPE, F24, BILANCIO, REDDITI;
- una pratica in anticipo;
- una in linea;
- una in ritardo operativo;
- una in attesa esterna simulata.

## 20. Criterio di accettazione UX

Aprendo il WMS senza spiegazioni, deve essere comprensibile:

- cosa deve fare ciascun Operatore;
- quali pratiche richiedono attenzione e perché;
- cosa deve controllare il Validatore;
- cosa governa il Manager;
- differenza fra stato reale e stato atteso rispetto alla scadenza;
- quali evidenze hanno prodotto le fasi precedenti.
