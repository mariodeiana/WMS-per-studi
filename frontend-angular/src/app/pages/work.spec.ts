...
describe('Operator queue',()=>{
  ...
  it('excludes inactive paths and orders executable work by deadline',()=>{page.tasks.set([task({code:'WAIT',active:false,due_date:'2025-01-01'}),task({code:'LATER',due_date:'2026-02-01'}),task({code:'FIRST'})]);expect(page.rows('ACTIVE').map(t=>t.code)).toEqual(['FIRST','LATER']);});
