...
function render(practice) {
  ...
  $("#responsible").textContent=practice.assignments.responsible; $("#operator").textContent=practice.assignments.operator;
  ...
  $("#tasks").innerHTML=practice.tasks.map(task=>`<div class="task ${task.status==='COMPLETATO'?'done':''}"><span class="check">${task.status==='COMPLETATO'?'✓':''}</span><div><div class="task-code">${task.code}</div><div class="task-title">${task.title}</div></div>${task.status==='COMPLETATO'&&canReopen?`<button class="reopen" data-reopen="${task.code}">Riapri</button>`:task.status!=='COMPLETATO'?`<button data-task="${task.code}">Completa</button>`:''}</div>`).join("");
  document.querySelectorAll("[data-task]").forEach(button=>button.onclick=()=>act(`/tasks/${button.dataset.task}/complete`,"operatore"));
  document.querySelectorAll("[data-reopen]").forEach(button=>button.onclick=()=>act(`/tasks/${button.dataset.reopen}/reopen`,"operatore"));
