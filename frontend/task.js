...
function renderAttachmentMeta(){
  ...
  container.innerHTML=selectedAttachments.map((item,i)=>`<div class="attachment-meta"><div class="attachment-title"><strong>📄 ${escapeHtml(item.file.name)}</strong><button type="button" class="icon-button" data-remove="${i}" title="Elimina l'allegato" aria-label="Elimina l'allegato ${escapeHtml(item.file.name)}">−</button></div><label>Descrizione<input data-description="${i}" type="text" value="${escapeHtml(item.description)}" placeholder="Descrizione o nota del documento"></label><label>Tipo documento <small>(opzionale)</small><input data-document-type="${i}" type="text" value="${escapeHtml(item.documentType)}" placeholder="es. F24, ricevuta, prospetto"></label></div>`).join("");
